...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(err) => match err {
            TicketNewError::TitleIsEmpty => panic!("Title cannot be empty"),
            TicketNewError::TitleTooLarge => panic!("Title cannot be longer than 50 bytes"),
            TicketNewError::DescriptionIsEmpty | TicketNewError::DescriptionTooLarge => {
...
impl Ticket {
    pub fn new(
        ...
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionTooLarge);
        }
